...
def biclustering_error(predicted_biclustering, reference_biclustering, num_rows, num_cols):
    """The biClustering Error (CE) external evaluation measure.

    CE computes the similarity between two subspace clusterings. This measure was originally
    introduced in (Patrikainen and Meila, 2006). In this package, it was implemented as
    presented in (Horta and Campello, 2014). This measure lies in the interval [0, 1], where
    values close to 1 indicate better biclustering solutions.
    ...
    """
    union_size = _calc_union_size(predicted_biclustering, reference_biclustering, num_rows, num_cols)
    dmax = _calc_dmax(predicted_biclustering, reference_biclustering)
    return float(dmax) / float(union_size)

def _calc_union_size(predicted_biclustering, reference_biclustering, num_rows, num_cols):
...
def _calc_dmax(predicted_biclustering, reference_biclustering):
